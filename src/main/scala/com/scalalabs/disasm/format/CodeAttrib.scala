...
import com.scalalabs.disasm.Preference
import com.scalalabs.disasm.classfile.RVA_annotation
import com.scalalabs.disasm.opcode._
...
class CodeAttrib(code: Array[Byte], val cpool: CPool) {
  ...
  def optCPRefW(code: Array[Byte], pc: Int): String = {
    val idx = readShort(code, pc)
    cpool.at(idx) match {
      case CONSTANT_Fieldref_info(clazzIdx,ntIdx) => cpool.classInfo(clazzIdx) +"." + cpool.nameTypeInfo(ntIdx) 
      case CONSTANT_Methodref_info(clazzIdx,ntIdx) => cpool.classInfo(clazzIdx) +"." + cpool.nameTypeInfo(ntIdx)
      case CONSTANT_InterfaceMethodref_info(clazzIdx,ntIdx) => cpool.classInfo(clazzIdx) +"." + cpool.nameTypeInfo(ntIdx)
      case CONSTANT_Class_info(idx) => cpool.utfString( idx )
      case x => x.toString
    }
//    cpool.at(idx).toString //mkString 
  ...
  def disAsmAttrib(code: Array[Byte], attr: Array[attribute_info]): List[AsmLine] = {
    ...
    val buff = scala.collection.mutable.ListBuffer[AsmLine]()
    var pc = 0;

    val lines = Map[Int, String]() //createLines(attr)
    val branches = createBranches(code)
    //    buff = buff + ">>branches count: " + branches.size + " " + branches


    while (pc < code.length) {
      val aline = new scala.collection.mutable.ListBuffer[String]
      if (Preference.displayLineNosInline) {
        lines.get(pc) match {
          case Some(label) => aline += label.toString
          case _ =>
        }
      }
      ...
      val pcLine = pc
      aline += (opcode(opVal)._1).toString
//      aline += (if (Preference.displayCodePoint) {
//        pc.toHexStr(4) + " " + opcode(opVal)._1
//      } else {
//        "     " + opcode(opVal)._1
//      })
      ...
      val handle = new DynamicProc {
        ...
        def addJump(idx: Int, jump: Int) {
          aline += "    " + idx + " " + branches.getOrElse(jump, jump.toHexStr(4))
        }

        def addDefault(default: Int) {
          aline += "    default: " + branches.getOrElse(default, default.toHexStr(4))
        }
      }

      opcode(opVal)._3 match {
        case Kind.Dynamic =>
          val (_, pc1:Int) = optDynamic(code, pc, handle)
          pc = pc1
        case _ =>
          aline += addKindInfo(opcode(opVal)._3, code, pc, branches)
  ...
  def addKindInfo(t: Kind.Value, code: Array[Byte], pc: Int, branches: Map[Int, String]): String = {
    ...
    t match {
      ...
      case Kind.BranchW =>
        ...
        branchStr(p, branches)
      case Kind.Byte => code(pc + 1).toHexStr(2) // 1,
      case Kind.CPRef =>
        val idx = readByte(code, pc)
        cpool.at(idx) match {
        	case CONSTANT_String_info(idx) => "'" + cpool.utfString( idx ) + "'"
        	case CONSTANT_Class_info(n) => cpool.utfString( idx )
        	case a => a.toString

        }
//        cpool.at(idx).toString //mkString // 1,
      case Kind.CPRefWUByte => {
        val opVal2 = readByte(code, pc)
        val opVal3 = readByte(code, pc + 1)
        cpool.at(opVal2).toString/*mkString*/ + " " + opVal3.toHexStr(2)
      }
      case Kind.CPRefWUByteZero => {
        // 4th byte is always 0
        val opVal2 = readShort(code, pc)
        /*val opVal3 =*/ readByte(code, pc + 2)
        cpool.at(opVal2) match {
        	case CONSTANT_InterfaceMethodref_info(clazzIdx,ntIdx) => cpool.classInfo(clazzIdx) +"." + cpool.nameTypeInfo(ntIdx)
        	case a => a.toString
        }
//        cpool.at(opVal2).toString/*mkString*/ + " " + opVal2
      }
      //        case Kind.Dynamic => optDynamic(code, pc) 
      case Kind.Local => readByte(code, pc).toHexStr(2) // 1,
      case Kind.LocalByte => readByte(code, pc).toHexStr(2) // 1,
      case Kind.Short => readShort(code, pc).toHexStr(4) // 2,
      case Kind.WideCPRefW => readByte(code, pc).toHexStr(2) + " " + readByte(code, pc).toHexStr(2) // 2,
      case Kind.WideCPRefWShort => readByte(code, pc).toHexStr(2) + " " + readByte(code, pc).toHexStr(2) // 2,
    ...
    while (pc < code.length) {
      ...
      opcode(opVal)._3 match {
        ...
        case Kind.Dynamic => {
          ...
          val (_, y:Int) = optDynamic(code, pc, handle)
          pc = y
        }
        case _ =>
      }
